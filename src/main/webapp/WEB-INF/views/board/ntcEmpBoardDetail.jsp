   ...
   <div class="container-fluid px-4">
      <div class="card">
         <div class="card-body" style="height: auto;">
            <a href="ntcEmpBoardList" style="color: #2E9AFE; font-size: 14px;">직원 공지사항 목록></a><br />
            <div class="detail-header mt-3">
               <div class="title">
                  <h5>${ntcBbs.nbTtl}</h5>
               </div>
               <div class="info">
                  <div>작성일: ${ntcBbs.nbPstdt}</div>
                  <div>조회수: ${ntcBbs.nbCount}</div>
               </div>
            </div>
            ...
            <c:if test="${user.deptNo eq 'A003'}">
            <div class="button">
               <br />
               <button class="btn btn-light btn-sm" type="button" onclick="location='ntcEmpBoardUpdate?nbNo=${ntcBbs.nbNo}'">수정</button>
               <button class="btn btn-light btn-sm" type="button" onclick="confirmDelete('${ntcBbs.nbNo}')">삭제</button>
            </div>
            </c:if><br /><br /><br /><hr />
			<div class="textarea-container" id="textareaContainer">
				<textarea id="qual" class="textarea-small" name="cmntCn" 
					placeholder="해당 공지사항의 댓글작성은 권한이 필요합니다.  공지사항에 대한 문의는 오호코리아 인사팀에 문의해주세요."></textarea>
			...
			<br />
			<!-- 댓글 시작 -->
			<c:forEach var="cmt" items="${ntcBbs.comentList}" varStatus="stat">
...
<script>
    		  ...
    		  if (result.isConfirmed) {
    		    Swal.fire({
    		      ...
    		      icon: "success"
                    ...
                    if (result.isConfirmed) {
    			  		location.href = 'ntcEmpBoardDelete?nbNo=' + nbNo;
                    }
                });
    		  }
    		});
    ...
    $.ajax({
        url: "/frcowner/ntcGetEmp",
        type: "get",
        dataType: "json",
        success: function(emp) {
            console.log("Logged in user: ", emp);
            userDeptNo = emp.deptNo;
            console.log("User Dept No:", userDeptNo); // 로그로 확인

            // 댓글 버튼 클릭 이벤트 핸들러
            $('#cmtBtn').on('click', function() {
                if (userDeptNo !== 'A003') {
                    alert("공지사항에 대한 문의는 오호코리아 인사팀에 문의해주세요.");
                    return;
                }

                let cmntCn = $("textarea[name='cmntCn']").val();
                let cmntCd = '${ntcBbs.nbNo}';
