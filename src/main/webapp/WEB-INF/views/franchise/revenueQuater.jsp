...
<main>
                        ...
                        <h1 class="page-header-title">
                            <div class="page-header-icon">
                                <i data-feather="bar-chart"></i>
                                   	...
                                   	분기별 매출 현황 / ${revenueList[0].frcsNm}
                        </h1>
    ...
    <div class="gTable">
        ...
        <div class="card-footer small text-muted">[분기별] 매출 총액 기준 통계 그래프</div>
            ...
            <tbody>
                <c:forEach var="revenue" items="${revenueList}" varStatus="stat">
                    <tr class="revenueArea" id="middel-table" data-bs-toggle="modal" data-bs-target="#exampleModalScrollable">
                        <td scope="row">${revenue.fsWrtQy}분기</td>
                        <td class="number-format"><em style="color: #584D4D;"><fmt:formatNumber type="number" maxFractionDigits="3" value="${revenue.totalQuaterFsAmt}" />원</em></td>
                        <td class="number-format"><em style="color: #5882FA;"><fmt:formatNumber type="number" maxFractionDigits="3" value="${revenue.totalQuaterFsCost}" />원</em></td>
                        <td class="number-format"><em style="color: #FF6666;"><fmt:formatNumber type="number" maxFractionDigits="3" value="${revenue.totalQuaterFsEarn}" />원</em></td>
                        <td class="number-format"><fmt:formatNumber type="number" maxFractionDigits="3" value="${revenue.totalQuaterFsOp}" />원</td>
                        <td class="number-format"><fmt:formatNumber type="number" maxFractionDigits="3" value="${revenue.totalQuaterFsNtslAmt}" />원</td>
                        <td class="number-format"><fmt:formatNumber type="number" maxFractionDigits="3" value="${revenue.totalQuaterFsMngAmt}" />원</td>
                        <td class="number-format"><em style="color: #0C9C56;"><fmt:formatNumber type="number" maxFractionDigits="3" value="${revenue.totalQuaterFsOp}" />원</em></td>
                        <td class="number-format"><em style="color: #5858FA;"><fmt:formatNumber type="number" maxFractionDigits="3" value="${revenue.totalQuaterFsExpense}" />원</em></td>
                    </tr>
                </c:forEach>
            </tbody>
        </table><hr/>
        <div class="small-text">
	        <b style="color:#848484">● 가맹점주님의 분기별 순이익 계산 방식은 아래와 같습니다. 각 항목을 다음과 같은 알파벳으로 정의하여 계산합니다.</b>&nbsp;&nbsp;&nbsp;
    ...
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content" style="width: 700px;">
            <div class="modal-header" style="display: flex; justify-content: space-between;">
			    <h5 class="modal-title" id="exampleModalScrollableTitle"><font color="white">분기별 매출 상세</font></h5>
            ...
            <div class="modal-body" id="myModel">
                    <div class="row">
                        <div class="col-lg-6 split-col" style="width: 100%;">
                            ...
                            <div class="card">
                                <div class="card-header">분기별 지출</div>
                            ...
                            <div class="card">
                                <div class="card-header">분기별 수익</div>
                                <div class="card-body">
                                    <table class="table">
...
<script>
var totalQuaterFsAmt = [];
var totalQuaterFsOp = [];
var fsWrtQy = ['1분기', '2분기', '3분기', '4분기'];
var myChart;
...
$(document).ready(function() {
    ...
    $('.dropdown').on('click', '.dropdown-item', function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                let newRevenueData = $(result).find('#revenueData').data('revenue');
                console.log("새로운 차트 데이터가 잘 들어갔는지 확인 -> " , newRevenueData);
                
                updateChart(newRevenueData);
            },
        ...
        $.ajax({
            url: "/frcowner/revenueQuaterChartAjax",
            type: "get",
            data: { fsWrtYear: fsWrtYear },
            dataType: "json",
            success: function(result) {
                ...
                // 데이터 추출
                let pgArr = result.revenueList;
                let pg2AmtArr = [];
                let pg2OpArr = [];
                for(let i=0; i<pgArr.length; i++){
                    pg2AmtArr.push(pgArr[i].totalQuaterFsAmt);
                    pg2OpArr.push(pgArr[i].totalQuaterFsOp);
                }
    ...
    <c:forEach var="revenue" items="${revenueList}">
    	totalQuaterFsAmt.push(${revenue.totalQuaterFsAmt});
    	totalQuaterFsOp.push(${revenue.totalQuaterFsOp});
    </c:forEach>
    ...
    myChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: '매출액',
                data: totalQuaterFsAmt,
                borderWidth: 1,
                backgroundColor: 'rgb(240, 56, 56,0.8)',
                borderColor: 'rgb(240, 56, 56,0.6)'
            },
            {
                label: '순이익',
                data: totalQuaterFsOp,
                borderWidth: 1,
                backgroundColor: 'rgb(16, 176, 111,0.8)',
                borderColor: 'rgb(16, 176, 111,0.6)'
            }]
        },
        options: {
            scales: {
...
function updateChart(newAmtData, newOpData) {
    ...
    newAmtData.forEach(function(data) {
    	totalQuaterFsAmt.push(data);
    });

    newOpData.forEach(function(data) {
    	totalQuaterFsOp.push(data);
    });

    // 차트 업데이트
    myChart.data.datasets[0].data = totalQuaterFsAmt;
    myChart.data.datasets[1].data = totalQuaterFsOp;
    myChart.update();
}
</script>
</body>
