...
public class ServiceLoggerAdvice {
   ...
   // Before어드바이스 : 조인 포인트(메소드) 전에 실행됨. 예외가 발생하는 경우만 제외하고 항상 실행됨
   @Before("execution(* kr.or.oho.*..*(..))")
   ...
   @AfterReturning("execution(* kr.or.oho.*..*(..))")
   ...
   @AfterThrowing(pointcut = "execution(* kr.or.oho.*..*(..))", throwing = "e")
   ...
   //어드파이스
   //조인 포인트(메서드)를 환료한 후에 실행함. 예외 발생이 되더라도 항상 실행 됨
   @After("execution(* kr.or.oho.*..*(..))")
